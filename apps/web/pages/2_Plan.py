...
# def _run_assess_and_maybe_apply():
#     verdict = assess_tweak(inputs_v11.get("goal",""), tweak or "")
#     st.session_state["tweak_verdict"] = verdict
#     v = verdict.get("verdict")
#     if v == "block":
#         st.error(verdict.get("rationale","This tweak is unsafe/contradicts evidence."))
#         return None
#     if v == "warn" and not st.session_state.get("confirm_apply_warn"):
#         st.warning(verdict.get("rationale","There are trade-offs. Click confirm to apply."))
...
# # Show verdict outcome (informational)
# v = st.session_state.get("tweak_verdict")
# if v:
#     (st.success if v["verdict"]=="ok" else st.warning if v["verdict"]=="warn" else st.error)(v["rationale"])
...
# # Sources remain server-side only (logged in plan generation), as requested.
